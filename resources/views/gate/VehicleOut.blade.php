			                ...
			                <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
			                    ...
			                    <tbody>
			                         @foreach ($car as $value)
			                    			<tr>
						                          <th>{{$value->car_plate}}</th>
						                          <th>{{$value->vehicle_model}}</th>
						                          <th>{{$value->vehicle_timein}}</th>
						                          <th align = 'center'><center>
						                          	<input type = 'checkbox' class = 'form-control' name = 'txt-check' id='validation' value = 'Yes'>
						                          </th></center>
						                          <th align = 'center'><center>
						                          	 <button class='btn btn-success' type='button' data-toggle='modal' data-target='#exitVehicle' onClick='exitVehicle();' name = 't2' id = 'car_id' value = '$car_id'><span class='glyphicon glyphicon-search'></span> View Details</button>
						                          </th></center>
						                        </tr>
                                @endforeach
			                    </tbody>
